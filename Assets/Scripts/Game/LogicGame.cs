...
public class LogicGame : MonoBehaviour
{
    ...
    void InitBubbles()
    {
        ...
        {
            tutorialManager.handClick.gameObject.SetActive(false);
        ...
        for (int i = 0; i < level.bubbles.Count; i++)
        {
            if (level.bubbles[i].hasChildren || level.bubbles[i].isChild)
            {
                numCountParentOrChild++;
            }
        }
    ...
    {
        indexBBQuestions.Clear();
        int c = level.countOfBBQuestion;
        while (c > 0)
        {
            int indexBBQuestion;
    ...
    void CreateBBFrozen()
    {
        listIndexBBFrozen.Clear();
        int c = level.countOfBBFrozen;
    ...
    void UseBoosterLightning()
    {
        ...
        do
        {
            ...
            if (indexHint != -1)
            {
                ...
                {
                    index = Random.Range(0, listBBShuffle.Count);
                }
            }
            ...
            for (int i = 0; i < listBBShuffle.Count; i++)
            {
                for (int j = i + 1; j < listBBShuffle.Count; j++)
                {
                    if (listBBShuffle[i].ID == listBBShuffle[index].ID && listBBShuffle[j].ID == listBBShuffle[index].ID
                        && i != index && j != index
                        && !listBBShuffle[i].hasChildren && !listBBShuffle[j].hasChildren && !listBBShuffle[index].hasChildren)
                    {
                        var g1 = listBBShuffle[i];
                        var g2 = listBBShuffle[j];
    ...
    void CheckEat()
    {
        ...
        for (int i = 0; i < listGOStored.Count - 2; ++i)
        {
                ...
                && listGOStored[i].ID == listGOStored[i + 1].ID && listGOStored[i + 1].ID == listGOStored[i + 2].ID)
            {
                ...
                starWinGame += comboSystem.comboCount * 1;
                isHint = false;
                SolveFrozenBB();
                ...
                g1.Move(g2.transform.parent, 0.3f, () =>
                {
                    ...
                    if (comboSystem.comboCount >= 2)
                    {
                        for (int i = 0; i < comboSystem.comboCount; i++)
                        {
    ...
    private void SolveFrozenBB()
    {
        ...
        if (indexLayerFrozen < 3)
        {
            ...
            listBBFrozens[0].particleFrozenBB.Play();
        }
    ...
    {
        for (int i = 0; i < listBBFrozens.Count; ++i)
        {
            for (int j = 0; j < listBBFrozens[i].layerFrozen.Count; ++j)
            {
                listBBFrozens[i].layerFrozen[j].SetActive(false);
            }
        }

        for (int i = 0; i < listBBQuestions.Count; ++i)
        {
            listBBQuestions[i].questionObject.SetActive(false);
    ...
    public void CheckWin()
    {
        if (!checkLose && !checkWin && listBB.Count <= 0)
        {
            timer.stopTimer = true;
            checkWin = true;
            logicUI.btnPause.interactable = false;
    ...
    void SolveBBFrozenWhenHint(Bubble bb)
    {
        if (bb.isFrozen)
        {
            for (int i = 0; i < bb.layerFrozen.Count; i++)
            {
                bb.layerFrozen[i].SetActive(false);
            }
        }
    }
    public void Undo()
    ...
    public void Freeze()
    {
        ...
        if (DataUseInGame.gameData.indexLevel == 4 && !DataUseInGame.gameData.isTutFreezeDone)
        {
            ...
            logicUI.tutBtn.ImageHint.SetActive(false);
            DataUseInGame.gameData.isTutFreezeDone = true;
            DataUseInGame.instance.SaveData();
            timer.stopTimer = false;
